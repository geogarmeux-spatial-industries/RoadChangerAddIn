...
namespace RoadChangerAddIn
{
    ...
    /// <summary>TDS spacing requirements; the circle diameter uses one of these.</summary>
    internal enum SizeReq { RoadSpacing, RoadBUA, CartSparse, CartDense }

    /// <summary>
    /// Shared state for the Sizing Cursor. The circle DIAMETER (meters) is the TDS
    /// spacing requirement for the current scale, and the reticle COLOR is the scale.
    /// The box, scale/requirement buttons, and the tool stay in sync via DiameterChanged
    /// (size) and ColorChanged (scale color) events.
    /// </summary>
    internal static class SizingCursorState
    {
        // Diameter (meters) per scale per requirement — from the TDS quick-reference cards.
        ...
        private static double _diameter = 300.0;
        ...
        /// <summary>Raised when the diameter changes (redraw + refresh the box).</summary>
        ...
        /// <summary>Circle diameter in meters (= the active spacing requirement).</summary>
        ...
        /// <summary>Set the active scale: changes color and re-applies the diameter for the current requirement.</summary>
        public static void SetScale(SizeScale scale)
        {
            CurrentScale = scale;
            _diameter = Table[scale][CurrentReq];
            ColorChanged?.Invoke(null, EventArgs.Empty);
            DiameterChanged?.Invoke(null, EventArgs.Empty);
        }

        /// <summary>Set the active requirement: diameter = that requirement at the current scale.</summary>
        public static void SetRequirement(SizeReq req)
        {
            CurrentReq = req;
            _diameter = Table[CurrentScale][req];
            DiameterChanged?.Invoke(null, EventArgs.Empty);
        }
